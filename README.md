...
WorkBuddy 记忆优化技能。解决三类问题：**记忆写错层级**（用户级 / 项目级混淆）、**记忆腐化**（重复、冲突、过时、超限）与**上下文膨胀**（主记忆文件随使用增长、撑爆上下文）。提供四项能力：写入层级自动判定、外部文件存储模式（降上下文）、记忆健康检查、每日日志 30 天留存蒸馏。
...
- **写入层级判定**：用户说"记住 / 保存"或 Agent 需持久化信息时，自动判断写入
  ...
  层级不确定时主动提问，不猜测。
- **外部文件存储模式（降上下文，核心架构）**：所有类型的记忆详细内容（环境坑、经验教训、偏好、项目决策、事实……）一律写入独立的 `notes/` 外部文件；
  主记忆文件只保留**轻量触发词 + 一句话摘要 + 指向外部文件的指针**（`- [<分类>:<关键词>] <摘要> → notes/路径.md`）作为检索索引。
  会话开始仅载入索引文件，命中关键词才读取对应外部文件，天然控制上下文占用。
- **记忆健康检查**：扫描各层字符上限、超 30 天日志、层内 / 跨层重复、过时标记、
  **报告冗余**（传入 `--reports-dir` 后，标出与本次报告逐字重复的条目）；
  并校验**外部文件存储模式**——标出指向不存在 notes 文件的**断链指针**、应外置却**内联过长**的条目、**未被引用的孤立 notes**；
...
python3 scripts/check_memory_health.py \
    ...
    --user-notes "<用户级 notes 目录，默认 <home>/memory_notes>" \
    --project-notes "<项目级 notes 目录，默认 <workspace>/.workbuddy/memory/notes>"
# 加 --json 输出结构化结果；--reports-dir / --user-notes / --project-notes 均可省略
# 脚本输出：各层字符数 vs 上限、超 30 天日志、层内/跨层重复、过时标记、报告冗余、
#          外部文件存储模式校验（断链指针 / 内联过长 / 孤立 notes）
```
